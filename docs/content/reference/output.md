...
describes what users get. Pick a format with `-o`, or let bri choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
bri <command> -o table   # aligned columns for reading
bri <command> -o jsonl   # one JSON object per line, for piping
bri <command> -o json    # a single JSON array
bri <command> -o csv     # spreadsheet friendly
bri <command> -o tsv     # tab-separated
bri <command> -o url     # just the URL column
bri <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
bri <command> --fields id,title,url
```
...
```bash
bri <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
bri <command>            # a table, because this is a terminal
bri <command> | wc -l    # JSONL, because this is a pipe
```
